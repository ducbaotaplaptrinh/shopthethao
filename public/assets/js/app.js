    ...
    if (searchInput && suggestionsDropdown) {
        ...
        searchInput.addEventListener('input', function() {
            ...
            debounceTimer = setTimeout(() => {
                fetch(`?page=search-suggest&keyword=${encodeURIComponent(query)}`)
                    .then(response => response.json())
                    .then(data => {
                        suggestionsDropdown.innerHTML = '';
                        if (data.length === 0) {
                            suggestionsDropdown.innerHTML = '<div class="p-3 text-center text-muted small">Không tìm thấy sản phẩm phù hợp</div>';
        ...
        searchInput.addEventListener('focus', function() {
            if (suggestionsDropdown.children.length > 0 && this.value.trim().length >= 2) {
                suggestionsDropdown.style.display = 'block';
            }
        });
    }
